...
function addStandardVehicleCombatRecordType()
	if not RecordDataManager.isRecordType("vehicle") then
		return;
	end

	local t = RecordDataManager.getCustomData("battle", "acceptdrop", { "npc" });
	if not StringManager.contains(t, "vehicle") then
		table.insert(t, "vehicle");
		RecordDataManager.setCustomData("battle", "acceptdrop", t);
	end
...
function handleStandardCombatAddFields(tCustom)
	...
	-- Link handling
	local sClass = tCustom.sClass or RecordDataManager.getRecordTypeDisplayClass(tCustom.sRecordType);
...
function addBattle(tCustom)
	...
	tCustom.sListPath = RecordDataManager.getCustomData("battle", "npclist", "npclist");
	...
	for _, nodeBattleEntry in ipairs(DB.getChildList(tCustom.nodeRecord, tCustom.sListPath)) do
		...
		if tBattleEntry and tBattleEntry.nodeRecord then
			for i = 1, tBattleEntry.nCount do
				...
				local sEntryRecordType = RecordDataManager.getRecordTypeFromRecordPath(t.sRecord);
...
function onBattleButtonAdd(w)
	...
	local sRecordType = RecordDataManager.getRecordTypeFromDisplayClass(w.getClass());
