...
function helperUpdateImageContextGetDataPaths()
	local tDataPaths = {};
	local sClass = RecordDataManager.getRecordTypeDisplayClass("image");
...
function helperSetRecordName(nodeSoundset, sRecordType, sRecord)
	...
	if DB.getValue(nodeSoundset, "name", "") == "" then
		local sNewName = string.format("%s - %s", RecordDataManager.getRecordTypeDisplayTextSingle(sRecordType), sName);
