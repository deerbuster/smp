...
function setActiveWidget(tokenCT, nodeCT, bActive)
	...
	local tWidget = {
		...
		displaytype = tData.bUnder and "underlay" or "",
	};
...
function getTokenActiveOptions()
	...
	if StringManager.startsWith(tData.sAsset, "under|") then
		tData.sAsset = tData.sAsset:sub(7);
		tData.bUnder = true;
	end
	return tData;
end
function setTokenActiveOptions(tData)
	...
	if tData.bUnder then
		OptionsManager.setOption("INITIND", string.format("under|%s", tData.sAsset));
	else
